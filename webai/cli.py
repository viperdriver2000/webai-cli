...
async def _run_single(provider_name: str, prompt: str, conf, raw: bool = False):
    """Run a single provider with live streaming output."""
    ProviderClass = get_provider(provider_name)
    profile_dir = Path(conf.profile_dir).expanduser() / provider_name
    browser = ProviderClass(profile_dir, conf.headless)
    try:
...
async def run_oneshot(provider_names: list[str], prompt: str, raw: bool = False,
    ...
    if not multi and not judge:
        await _run_single(provider_names[0], prompt, conf, raw)
        return
    ...
    # Judge mode — send all results to a final provider for summary
    if judge:
        answers = "\n\n".join(
        ...
        if raw:
            ...
            console.print(f"\n[bold yellow]━━━ JUDGE: {judge.upper()} ━━━[/bold yellow]")
        await _run_single(judge, judge_prompt, conf, raw)
...
def main():
    ...
    if args.judge and args.judge not in available:
        p.error(f"Unknown judge provider: {args.judge!r}. Available: {', '.join(available)}")
